...
public class WheelView extends View {
    ...
    /** 当前条目中的文字颜色 */
    private static final int VALUE_TEXT_COLOR = Color.parseColor("#aae3b8");

    /** 非当前条目的文字颜色 */
    private static final int ITEMS_TEXT_COLOR = Color.parseColor("#4c4c4c");
   ...
   // private static final int[] SHADOWS_COLORS = new int[] { 0xFF111111, 0x00AAAAAA, 0x00AAAAAA };
    ...
    /** 字体大小 */
    private  int text_size = 24;//static final
   private int selected_text_size=24;
    /** 顶部 和 底部 条目的隐藏大小, 
     * 如果是正数 会隐藏一部份, 
     * 0 顶部 和 底部的字正好紧贴 边缘, 
     * 负数时 顶部和底部 与 字有一定间距 */
    private  int item_offset = 4;//TEXT_SIZE / 5;//static final
    ...
    /** WheelView 适配器 */
 //   private WheelViewAdapter adapter = null;
...
//    private GradientDrawable bottomShadow;
    ...
    public void setText_size(int text_size,int selected_text_size) {
		this.text_size = text_size;
		this.selected_text_size=selected_text_size;
    ...
    private void initResourcesIfNecessary() {
        ...
        if (valuePaint == null) {
            ...
            valuePaint.setTextSize(selected_text_size);
...
//        setBackgroundColor(Color.parseColor("#1c4861"));
    }
    ...
    private int getMaxTextLength() {
        ...
        int adapterLength =adapter.getItemsCount();// adapter.getMaximumLength();
        if (adapterLength > 0) {
        ...
        for (int i = Math.max(currentItem - addItems, 0); i < Math.min(currentItem + visibleItems,
            ...
            String text = adapter.getItemText(i).toString();//
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        ...
        if (!isScrollingPerformed && (valueLayout == null || valueLayout.getWidth() > widthItems)) {
            String text = getAdapter() != null ? getAdapter().getItemText(currentItem).toString()//getAdapter().getItem(currentItem)
            		: null;
